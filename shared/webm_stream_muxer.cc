...
WebMStreamMuxer::WebMStreamMuxer()
    : audio_track_num_(0),
      video_track_num_(0),
      initialized_(false) {
}
...
int WebMStreamMuxer::Init(uint64 maxClusterDuration) {
  ...
  ptr_segment_->set_max_cluster_duration(maxClusterDuration * 1000000ULL); // 1s
  ...
  ptr_segment_info->set_writing_app("WebMStreamMuxer");
  initialized_ = true;
  return kSuccess;
}

int WebMStreamMuxer::AddAudioTrack(int sample_rate, int channels,
...
bool WebMStreamMuxer::AddContentEncKeyId(uint64 track_num,
...
int WebMStreamMuxer::AddVideoTrack(int width, int height) {
...
bool WebMStreamMuxer::SetMuxingApp(const std::string& muxing_app) {
  using mkvmuxer::SegmentInfo;
  SegmentInfo* const ptr_segment_info = ptr_segment_->GetSegmentInfo();
  if (!ptr_segment_info) {
...
bool WebMStreamMuxer::SetWritingApp(const std::string& writing_app) {
  using mkvmuxer::SegmentInfo;
  SegmentInfo* const ptr_segment_info = ptr_segment_->GetSegmentInfo();
  if (!ptr_segment_info) {
...
int WebMStreamMuxer::WriteAudioFrame(const uint8* data, size_t size,
...
int WebMStreamMuxer::WriteVideoFrame(const uint8* data, size_t size,
                                   uint64 timestamp_ns, bool is_key ) {
  if (video_track_num_ == 0) {
...
int WebMStreamMuxer::WriteFrame(const uint8* data, size_t size,
...
// greater than 0.
bool WebMStreamMuxer::ChunkReady(int32* ptr_chunk_length) {
  if (ptr_chunk_length) {
...
int WebMStreamMuxer::ReadChunk(int32 buffer_capacity, uint8* ptr_buf) {
  if (!ptr_buf) {
